...
export async function JoinGroup(props) {
  try {
    ...
    const response = await axios.patch(
      apiGroupJoin + paramsCode.get("code"),
      {
...
export async function GetGroupDetail(props) {
  try {
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_DOMAIN_BE}` + `/group/id/${props._id}/detail`,
      {
        withCredentials: true,
      },
    );
...
export async function LeaveGroup(props) {
  try {
    const response = await axios.delete(
      `${process.env.NEXT_PUBLIC_DOMAIN_BE}` + `/group/leave/${props._id}`,
      {
        withCredentials: true,
      },
    );
...
export async function UpdateDescription(props) {
  try {
    const response = await axios.patch(
      `${process.env.NEXT_PUBLIC_DOMAIN_BE}` +
        `/group/id/${props._id}/description`,
      {
...
export async function RegenerateCode(props) {
  try {
    const response = await axios.patch(
      `${process.env.NEXT_PUBLIC_DOMAIN_BE}` +
        `/group/id/${props._id}/generate-code`,
      {
        empty: "EMPTY",
      },
      {
